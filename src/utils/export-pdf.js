...
function drawTables(doc, inputs, outputs, pageW, y, t, lang) {
    ...
    const baseStyle = {
        styles: { fontSize: 6, cellPadding: 2, lineColor: C.border, lineWidth: 0.2, overflow: 'ellipsize', cellWidth: 'wrap',},
    ...
    autoTable(doc, {
        ...
        body: [
            ...
            [t('equipment.electrolyzer.maintenance_percent_capex'), `${fmt(inputs.selectedElectrolyzer.maintenance_percent_capex, lang)} ${t('equipment.electrolyzer.maintenanceUnit')}`],
            ...
            inputs.isCompressorNeeded ? [] : null,
            inputs.isCompressorNeeded ? [t('export.compressor'), t(inputs.selectedCompressor.name)] : null,
            inputs.isCompressorNeeded ? [t('compressorSetup.compressorType'),  t(inputs.selectedCompressor.type)] : null,
            inputs.isCompressorNeeded ? [t('compressorSetup.purchasePrice.label'), `${fmt(inputs.selectedCompressor.price, lang, 0)} ${t('units.eur')}`] : null,
            inputs.isCompressorNeeded ? [t('compressorSetup.hydrogenToCompress.label'), inputs.massToCompress === -1 ? 'Auto' : `${fmt(inputs.massToCompress, lang, 0)} ${t('units.kg_per_year')}`] : null,
            inputs.isCompressorNeeded ? [t('export.ownedUnits'), inputs.compressorSettings.owned] : null,
            inputs.isCompressorNeeded ? [t('compressorSetup.operatingTime.label'), `${fmt(inputs.compressorSettings.operatingTime.value, lang)} ${t(inputs.compressorSettings.operatingTime.unit.label)}`] : null,            
        ].filter(Boolean),
